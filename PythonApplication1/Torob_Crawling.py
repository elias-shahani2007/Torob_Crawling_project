...
class TorobScraperApp(ctk.CTk):
  ...
  def create_widgets(self):
    ...
    self.lbl_max_results = ctk.CTkLabel(
        self.frame_rows, text='تعداد نتایج ترب:'
    )
    self.lbl_max_results.pack(side='right', padx=(2, 5), pady=10)

    self.entry_max_results = ctk.CTkEntry(self.frame_rows, width=50)
    self.entry_max_results.insert(0, '18')
    self.entry_max_results.pack(side='right', padx=(0, 10), pady=10)
  ...
  def start_scraping_thread(self):
    ...
    try:
      ...
      if start_row < 1 or end_row < start_row or max_results < 1:
        raise ValueError()
    except ValueError:
  ...
  def run_scraper(self, start_row, end_row, max_results, is_headless):
    try:
      ...
      with sync_playwright() as p:
        ...
        # --- پرسش از کاربر جهت ثبت اسکرین‌شات نمودار تغییر قیمت ---
        ask_price_chart = messagebox.askyesno(
            'افزودن نمودار تغییر قیمت',
            'استخراج اولیه با موفقیت تمام شد.\n\nآیا تمایل دارید'
            ' اسکرین‌شات نمودار تغییر قیمت محصولات برتر نیز استخراج شده و به'
            ' شیت دوم اضافه شود؟',
        )

        if ask_price_chart and TorobChartModule:
