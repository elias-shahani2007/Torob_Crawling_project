...
class TorobScraperApp(ctk.CTk):
  # طبق تغییر ۲: تعداد نتایج ترب دیگر قابل ویرایش توسط کاربر نیست و ثابت است
  MAX_TOROB_RESULTS = 10
  ...
  def start_scraping_thread(self):
    ...
    threading.Thread(
        ...
        args=(start_row, end_row, self.MAX_TOROB_RESULTS, is_headless),
        daemon=True,
    ).start()
  ...
  def run_scraper(self, start_row, end_row, max_results, is_headless):
    try:
      ...
      # --- تغییر ۳: ماژول نمودار و شمارندهٔ ردیف تصویر، یک‌بار قبل از حلقهٔ اصلی ساخته می‌شن ---
      chart_module = TorobChartModule(excel_filename) if TorobChartModule else None
      image_row = 2
      ...
      with sync_playwright() as p:
        ...
        for idx, (_, row) in enumerate(sliced_df.iterrows()):
          ...
          # --- تغییر ۱: تا وقتی نتیجه پیدا نشه صبر می‌کنه و دوباره همین محصول را امتحان می‌کند ---
          while True:
            if self.stop_requested:
              break
          ...
          # --- تغییر ۳: اسکرین‌شات نمودار همین محصول، بلافاصله و بدون پرسش از کاربر ---
          if chart_module and target_results:
            ...
            if selected_prk:
              ...
              try:
                page.goto(
                    prod_url, wait_until='domcontentloaded', timeout=30000
                )
                time.sleep(2)
                ...
                if chart_btn.count() > 0:
                  ...
                  chart_module.add_chart_image(
                      product_name, img_filename, image_row
                  )
                  image_row += 15
